...
type Client = { cliente_id:string; razao_social:string; nome_fantasia:string; cnpj:string; email_principal:string; status:string; valor_mensal:string };
...
export default function Home() {
  ...
  async function generateBatch() {
    if (!selected.size) return setNotice("Selecione pelo menos um cliente.");
    setGenerating(true);
    try {
      ...
      setNotice(`${ok} cobrança(s) gerada(s)${failed ? ` e ${failed} não gerada(s)` : ""}.`);
      setSelected(new Set());
      await loadAll();
  ...
  const filtered = useMemo(() => clients.filter((client) => {
    ...
    return client.status === "ATIVO" && text.includes(search.toLowerCase());
  }), [clients, search]);
  ...
  return <main className="app-shell">
    ...
    <section className="content">
      ...
      <section className="batch-card" id="clientes"><div className="section-title"><div><p className="eyebrow">NOVO LOTE</p><h2>Gerar cobranças</h2></div><span className="selection-count">{selected.size} selecionado(s)</span></div><div className="controls"><label>Competência<input type="month" value={competence} onChange={(event) => setCompetence(event.target.value)} /></label><fieldset><legend>Vencimento</legend><label className={dueDay === 10 ? "choice checked" : "choice"}><input type="radio" checked={dueDay === 10} onChange={() => setDueDay(10)} /> Dia 10</label><label className={dueDay === 28 ? "choice checked" : "choice"}><input type="radio" checked={dueDay === 28} onChange={() => setDueDay(28)} /> Dia 28</label></fieldset><button className="primary" onClick={generateBatch} disabled={generating}>{generating ? "Gerando…" : "Gerar cobranças"}</button></div>
